...
#include "matrix_hamilton2.h"

long long generate_all_signatures(unsigned long long dimension, int width, unsigned long long *all_signatures, unsigned long long **valid_signatures, long long *counts);
void fill_transfer_matrix(unsigned int **transfer_matrix, int width, unsigned long long **valid_signatures, long long *counts, unsigned short *lengths);
...
int main() {
    ...
    unsigned long long dimension = 6 * pow(4, width-1);
    ...
    dimension = generate_all_signatures(dimension, width, all_signatures, valid_signatures, counts);
	...
	fill_transfer_matrix(transfer_matrix, width, valid_signatures, counts, lengths);
...
long long generate_all_signatures(unsigned long long dimension, int width, unsigned long long *all_signatures, unsigned long long **valid_signatures, long long *counts){
	...
	for(int i = 0; i<GROUP_SIZE; i++){
		...
		construct_signatures(all_signatures, dimension, width, n);
		
		count = count_valid_signatures(all_signatures, dimension/GROUP_SIZE, width);
		
		valid_signatures[i] = (unsigned long long*)malloc(count * sizeof(unsigned long long));

		save_valid_signatures(valid_signatures[i], all_signatures, dimension/GROUP_SIZE, width);
...
* saves the corresponding indizes in the transfer matrix.
*/
void fill_transfer_matrix(unsigned int **transfer_matrix, int width, unsigned long long **valid_signatures, long long *counts, unsigned short *lengths){
	...
	for(int i = 0; i<GROUP_SIZE; i++){
		for(long long j = 0; j<counts[i]; j++){
			...
			unsigned int length = compute_successor_signatures(successor_signatures, valid_signatures[i][j], width);
