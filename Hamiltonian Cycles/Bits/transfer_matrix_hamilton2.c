#include "transfer_matrix_hamilton2.h"
...
void get_string_rep(unsigned long long signature, int w){
	...
	for(int i = 0; i<w+1; i++){
		...
		else if((signature & base) == base/3){
			printf("B");
		}
		else if((signature & base) == 2*base/3){
			printf("O");
		}
		else{
			printf("X");
...
long long binary_search(unsigned long long *valid_signatures, unsigned long long signature, long long l, long long r, int width){
    if (r >= l) {
        long long m = l + (r - l) / 2;
...
long long find_index(unsigned long long **valid_signatures, unsigned long long signature, long long *counts, int width){
	long long count = 0;
...
bool is_valid_signature(unsigned long long signature, int width){
	
	unsigned long long base = 3*pow(4, width);
	...
	for(int i = 0; i<width+1; i++){
		...
		else if((sig_copy & base) == pow(4, width)){
			number_b++;
		}
    ...
    return number_a != 0 && (number_a == number_b) && ((signature & 3) != 2);
}

void construct_signatures(unsigned long long *signatures, unsigned long long dimension, int width, unsigned long long n){
	//              A, B, O, X
    int states[] = {0, 1, 2, 3};	
    ...
    //Set first State
    for(int i = 0; i<d; i++){
        signatures[i] = start * pow(4, width);
    }
    ...
    for(int i = 0; i < width-1; i++){
        ...
        for(unsigned long long j = 0; j<d; j++){
			...
			signatures[j] = signatures[j] + states[state_number] * pow(4, width-i-1);			
            current--;
        }
        period = period/4;
    }
...
void save_valid_signatures(unsigned long long *valid_signatures, unsigned long long *signatures, unsigned long long dimension, int width){
...
long long count_valid_signatures(unsigned long long *valid_signatures, unsigned long long dimension, int width){
    long long count = 0;
...
unsigned long long change_to_a(unsigned long long signature, int number_of_a, int width){
	unsigned long long sig_copy = signature;
	...
	for(int i = 0; i<width+1; i++){
		if((sig_copy & base) == 0){
			...
			number_of_a --;
            if(number_of_a == 0){
				result -= (unsigned long long)pow(4, width-i);
                break;
            }
		}
...
unsigned long long change_to_b(unsigned long long signature, int number_of_b, int width){
...
bool are_on_the_same_path(unsigned long long signature, int index_a, int offset_b, int width){
	unsigned long long base = 3;
