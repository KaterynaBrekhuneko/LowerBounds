...
int calculate_eigenvalue2(unsigned long long dimension, unsigned long long **matrix, double *eig, double *vec, unsigned short *lengths,
    unsigned long max_iter, double epsilon)
{
    ...
    do
    {
        m_v_mul(dimension, matrix, vec, tmp, lengths);
        ...
        *eig = v_v_mul(dimension, tmp, vec);
        ...
        if(!*eig)
        {
			 ...
			 * Generate another random start vector and start iterating again*/
            do{
				v_init(dimension, vec);
			}				
			while(v_abs(dimension, vec) < 0.5);
			...
			// Reset the error margin, so that the loop continues. 
            delta = 2.0 * epsilon; 
			i = 0;
        }
        else
	...
	// Iteration will stop when eather the sufficient error margin or the max number of iterations has been reached
...
int calculate_eigenvalue(unsigned long long dimension, unsigned long long **matrix, double *eig, double *vec, unsigned short *lengths){
    return calculate_eigenvalue2(dimension, matrix, eig, vec, lengths, MAX_ITER, EPSILON);
}
...
double v_mr_mul(unsigned long long *v1, double *v2, unsigned short length){	
    double sum = 0;
...
void m_v_mul(unsigned long long dimension, unsigned long long **matrix, double *v, double *vresult, unsigned short *lengths){   
	for(unsigned long long row = 0; row < dimension; row++){
        vresult[row] = v_mr_mul(matrix[row], v, lengths[row]);
    }
}