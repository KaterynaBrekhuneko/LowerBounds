...
int calculate_eigenvalue2(unsigned long long dimension, unsigned long long **valid_signatures, long long *counts, int w, double *eig, double *vec,
    ...
    do
    {
        ...
        m_v_mul(dimension, tmp, valid_signatures, counts, vec, w);

        *eig = v_v_mul(dimension, tmp, vec);

        if(!*eig)
        {
			/* We choose the start vector perpendicular to the eigenvector 
    ...
    //Veryfy if the maximum number of iterations was not exceeded
    return (i >= max_iter) ? 1 : 0;
}

int calculate_eigenvalue(unsigned long long dimension, unsigned long long **valid_signatures, long long *counts, int w, double *eig, double *vec){
    return calculate_eigenvalue2(dimension, valid_signatures, counts, w, eig, vec, MAX_ITER, EPSILON);
}
...
void m_v_mul(unsigned long long dimension, double *vresult, unsigned long long **valid_signatures, long long *counts, double *v, int w)
{
